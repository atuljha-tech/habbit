...
import HypemanBanner from "@/components/HypemanBanner"
import { useHypeman } from "@/hooks/useHypeman"
import { 
  Plus, 
  Flame, 
  Trophy, 
...
export default function HabitsPage() {
  ...
  // 👇 STEP 1: Add Hypeman hook
  const { currentMessage, hype, clearHype } = useHypeman()
  ...
  // 👇 STEP 2: Update createHabit function with Hypeman
  async function createHabit(e: React.FormEvent) {
    ...
    try {
      ...
      // 🎉 HYPE THE USER! This triggers the banner
      await hype("created", title)
  ...
  // 👇 STEP 3: Add delete function with Hypeman
  async function deleteHabit(habitId: string, habitTitle: string) {
    // Ask for confirmation first
    if (!confirm(`Are you sure you want to delete "${habitTitle}"?`)) {
      return
    }
  ...
  return (
    <>
      {/* 👇 STEP 4: Add Hypeman Banner at the top */}
      {currentMessage && (
        <HypemanBanner 
                        ...
                        <div className="mt-6 pt-4 border-t-4 border-black flex flex-col gap-3">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                              ...
                              {/* 👇 DELETE BUTTON - FIXED: Now properly inside the map function */}
                              <button 
                                onClick={() => deleteHabit(habit._id, habit.title)}
                                className="border-3 border-black bg-white px-3 py-1 text-xs font-bold hover:bg-[#FF7AC6] transition-colors"
                              >
                                ✕
                              </button>
                            </div>
